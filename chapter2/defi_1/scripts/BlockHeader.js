...
require(["crypto-js"], function (CryptoJS) {
	...
	function ComputeTXID(txBitstream){
		...
		let tab = CryptoJS.enc.Hex.parse(txBitstream);

		let hashBlock = CryptoJS.SHA256(tab);
		let hashBlock2 = CryptoJS.SHA256(hashBlock);

	  	return hashBlock2.toString(CryptoJS.enc.Hex);
	}
	...
	class Block{
		constructor(bitstream){
			...
			for (let i = 0; i < this.nbTX; i++){
				...
				let txid = ComputeTXID(bitstream.slice(k, k + tx.getParsedLength()));
				k += tx.getParsedLength();
				this.Transactions.push(tx);
				this.TXIDs.push(SwapEndian(txid, true));
			}
	...
	class Transaction{
		...
		toString(){
			...
			for (let i = 0; i < this.inputCount; i++){
				...
				out += "	SCRIPTSIG = " + this.Inputs[i].ScriptSig + "\n";
